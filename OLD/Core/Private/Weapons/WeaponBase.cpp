...
AWeaponBase::AWeaponBase()
{
	...
	//Replicate weapon across everything?
	bReplicates=true;
	//SetReplicates(true);
}
...
void AWeaponBase::ServerOnAttack_Implementation()
{
	...
	if (bIsProjectileWeapon)
	{
		...
		AttackComponent->FireProjectile<AProjectile>(GetWorld(), SpawnLocation, SpawnRotation, this, GetInstigator());
	}
	else
	{
		if (GEngine && Player)
		{
			GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, TEXT("Hitscan"));
		}
	}
